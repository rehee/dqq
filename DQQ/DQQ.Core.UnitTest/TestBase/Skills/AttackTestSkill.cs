...
namespace DQQ.UnitTest.TestBase.Skills
{
	public class AttackTestSkill : SkillComponent
	{
		public async Task<bool> CastSkill(ComponentTickParameter? parameter)
		{
			...
			parameter.SelectedTarget.CurrentHP = parameter.SelectedTarget.CurrentHP - 100;
			if (parameter.SelectedTarget.CurrentHP <= 0)
			{
				parameter.SelectedTarget.CurrentHP = 0;
			}
			//parameter!.SelectedTarget!.TakeDamage(
			//	ComponentTickParameter.New(parameter, parameter.From, parameter!.SelectedTarget, null, null, DamageDeal.New(100))
			//	);
			return true;
		}
