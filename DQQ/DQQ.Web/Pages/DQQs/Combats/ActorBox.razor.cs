	...
	public class TextFlootCheck
	{
		...
		public TextFloatParameter GetParameter()
		{
			return new TextFloatParameter
			{
				...
				Text = Skill != null ? Skill?.SkillName : Healing != null ? $"{Healing?.HealingDone} {(Healing?.Absorbe>0?$"Absorbe({Healing?.Absorbe})":"")} {(Healing?.OverHeal>0?$"Over Heal({Healing?.OverHeal})":"")}" : Damage?.DisplayDamage ,
			};
		}
