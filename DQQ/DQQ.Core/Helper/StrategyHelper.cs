...
namespace DQQ.Helper
{
  public static class StrategyHelper
  {
    ...
    public static StrategeCheckResult? MatchSkillStrategyTarget(this SkillStrategy strategy, ITarget? caster, IEnumerable<ITarget>? targets, IMap? map)
    {
      ...
      {
        return StrategeCheckResult.New(true, checkTarget);
      }
      return null;
    }
    public static StrategeCheckResult? MatchSkillStrategyParty(this SkillStrategy strategy, ITarget? caster, IEnumerable<ITarget>? targets, IMap? map)
    {
      strategy.CheckTarget = strategy?.Condition == EnumStrategyCondition.Enemies ? EnumTarget.Target : EnumTarget.Self;
      ITarget? skillTarget = null;

      IEnumerable<ITarget>? checkingTarget = null;
      if (strategy?.Condition == EnumStrategyCondition.Enemies)
      {
      ...
      switch (strategy.PartyStrategy)
      {
        case EnumStrategyParty.AliveNumber:
          long aliveNumber = aliveTargets.Count();
          var checkResult = strategy.Compare.NumberCompare(aliveTargets.Count(), strategy.Value);
          if (checkResult == true)
          {
